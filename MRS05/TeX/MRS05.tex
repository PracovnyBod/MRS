...
\def\oznacenieCasti{MRS05 - ZS2026}
...
Uvedený model \eqref{PohRovKyvadla} opisuje vstupno-výstupné správanie sa dynamického systému, kde vstupom je externý moment sily $u(t)$ a~výstupom je uhol $\varphi(t)$, avšak budeme pracovať aj s opisom systému v~\uv{stavovom priestore}.
...
\begin{lstlisting}[language=Matlab, title=Funkcia \texttt{fcnF} (napr. v samostatnom súbore fcnF.m)]
function dotx = fcnF(t,x)
...
\begin{lstlisting}[language=Matlab, title=Časť súbora hlSkript.m, name=hlSkript]
[t,x] = ode45(@(t,x) fcnF(t,x), timeVect, [0.25; 0]);
\end{lstlisting}
